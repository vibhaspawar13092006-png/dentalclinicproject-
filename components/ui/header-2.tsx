...
export function Header() {
	...
	return (
		...
		>
			...
			<div
				className={cn(
					'bg-background/95 backdrop-blur-md fixed top-16 right-0 bottom-0 left-0 z-50 flex flex-col overflow-hidden border-y md:hidden transition-all duration-300',
			...
			>
				<div
					...
					className={cn(
						...
						'flex h-[calc(100vh-4rem)] w-full flex-col justify-between gap-y-2 p-6 pb-20',
					)}
				>
					<div className="grid gap-y-3">
						{links.map((link) => (
							<a
								...
								className="rounded-xl px-4 py-3 text-lg font-medium text-muted-foreground hover:bg-secondary hover:text-foreground transition-all"
								href={link.href}
							>
								{link.label}
							</a>
					...
					<div className="flex flex-col gap-3">
						...
						<Show when="signed-out">
							<SignInButton mode="modal" fallbackRedirectUrl="/dashboard">
								<Button variant="outline" className="w-full rounded-full py-5 text-sm">
									Sign In
								</Button>
							</SignInButton>
							<SignUpButton mode="modal" fallbackRedirectUrl="/dashboard">
								<Button className="w-full rounded-full py-5 text-sm">
									Sign Up
								</Button>
							</SignUpButton>
						</Show>
